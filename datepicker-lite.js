...
class DatePickerLite extends GestureEventListeners(PolymerElement) {
  ...
  maxDateValidation() {
    if (this.maxDate && this.value) {
      let valid = dayjs(this.value, controlFormat) <= this.maxDate;
      if (!valid) {
  ...
  minDateValidation() {
    if (this.minDate && this.value) {
      let valid = dayjs(this.value, controlFormat) >= this.minDate;
      if (!valid) {
  ...
  _valueChanged(newValue) {
    ...
    if (this.inputDateFormat) {
      const formattedDate = dayjs(newValue, this.inputDateFormat, true);
  ...
  formatDateForDisplay(selectedDt, readonly) {
    ...
    return dayjs(selectedDt, controlFormat).format(this.selectedDateDisplayFormat);
  }
